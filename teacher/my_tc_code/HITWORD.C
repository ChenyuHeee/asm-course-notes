...
{
   "armadillo", "bull", "cat", "dog", "eagle", 
   "fox", "giraffe", "hyena", "iguana", "jaguar", 
   "kangaroo", "llama", "monkey", "nightingale", 
   "octopus", "parrot", "quail", "roadrunner", 
   "seagull", "timberman", "urchin", "vulture", 
...
void generate_one_word(void)
{
   ...
   for(j=0; j<screen_words; j++) /* 根据屏幕上已显示单词的首字母设置mark */
   {                             /* 注意屏幕上每个单词的首字母一定不相同 */
      mark[w[j].letter[0] - 'a'] = 1; /* 若w[j]已在屏幕上, 则首字母对应的mark标为1, */
...
void show_one_word(int i)
{
   ...
   if(w[i].status >= 1) /* 1=hit, 2=to_be_destroyed */
   {
      for(j=0; j<w[i].hit_len; j++) 
         show_char(w[i].x+j, w[i].y, w[i].letter[j], 0x02); /* 已打字母显示绿色 */
      for(j=w[i].hit_len; j<n; j++)
         show_char(w[i].x+j, w[i].y, w[i].letter[j], 0x07); /* 没打字母显示白色 */
   }
   else /* w[i].status == 0, 该单词没有打中 */
   {
      for(j=0; j<n; j++)
         show_char(w[i].x+j, w[i].y, w[i].letter[j], 0x07); /* 用白色显示整个单词 */
   }
}
...
void move_words_down(void)
{
   ...
   for(i=0; i<screen_words; i++) /* 屏幕上每个单词的y坐标加1, 表示向下移动一行 */
   {
      w[i].y++;
   }
   if(w[0].y >= 24) /* 若w[0]已掉到屏幕外 */
   {
      disable(); /* To ensure that there is no occurring of int_9h()
                    ...
                    If we do not disable interrupts here, the int_9h() occurring 
                    between memcpy() and hit-- will get the wrong w[hit].
                    disable()相当于汇编语言中的cli指令
                   */
      memcpy(&w[0], &w[1], sizeof(w[0])*(screen_words-1)); /* 删除w[0]这个单词 */
      screen_words--;
      words_lost++;
      if(hit != -1) /* 打中单词的下标 */
         hit--;     /* 也要同步减1 */
...
void draw_bullet(int i)
{
   ...
   show_char(w[i].bx, w[i].by, 0x18, 0x0C); /* 0x18=ascii of bullet, 0x0C=bright RED  */
   /* 在当前子弹坐标处画子弹 */
...
void interrupt int_8h(void)
{
   ...
   {
      draw_bullet(i);
      if(w[i].by < w[i].y) /* the word has been hit by the bullet, 子弹已向上穿过单词 */
      {
         memcpy(&w[i], &w[i+1], sizeof(w[0])*(screen_words-1-i)); /* delete the word */
         if(hit != -1 && hit > i)
...
{
   w[i].bx = w[i].x + strlen(w[i].letter)/2; /* 子弹的当前x坐标设在w[i]这个单词的中间 */
...
void interrupt int_9h(void)
{
   ...
   {
      if(w[hit].letter[hit_len] == ascii) /* 判断w[hit]的剩余字母的首字母有没有打中 */
      {
         hit_len++;
         w[hit].hit_len = hit_len;
         if(!updating)          /* 只有当move_words_down()函数没在更新屏幕内容时, */
            show_one_word(hit); /* 才可以在int_9h()里面重画w[hit]更新刚打的字母颜色;
                                   若当前move_words_down()函数正在更新屏幕内容,
                                   则本次int_9h()函数暂时不更新刚打的字母颜色,
                                   等到下次int_9h()时再更新。
                                 */
         if(hit_len == strlen(w[hit].letter)) /* 若w[hit]的全部字母都已打完 */
         {
   ...
   else /* if the word is hit for the 1st time */
   {
      for(i=0; i<screen_words; i++)
      {
         if(w[i].status==2) /* skip the word to be destroyed */
            continue;       /* bug reported by 张帅辰 */
